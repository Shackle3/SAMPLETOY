...
const double NOTE_FREQUENCIES[88] = {
...
//Functions:
//
//Assuming midiIn is >= 21, translates a midi code into a frequency.
double midi_frequency_translator(int midiNoteCode){
    return NOTE_FREQUENCIES[midiNoteCode - 21];
}
...
upair32 callGenerateForEvent(generator* target, const uint8_t input_note_midi_code, float* note_phase_pos){
    ...
    float deltaphase = (2 * PI * midi_frequency_translator(input_note_midi_code)) * SAMPLE_DURATION;
    switch(target->generator_type){
        case MATHSYNTH:
            temp_output_container = mathgen_generate(target->define_wave, *note_phase_pos, input_note_midi_code);
            *note_phase_pos = (float) fmod((*note_phase_pos + deltaphase), (2 * PI));
